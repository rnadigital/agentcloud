use crate::vector_dbs::models::{CollectionCreate, CollectionsResult, Point, ScrollResults, SearchRequest, StorageSize, VectorDatabaseError, VectorDatabaseStatus};
...
impl VectorDatabase for QdrantClient {
    ...
    async fn delete_collection(
        ...
        match self.check_collection_exists(&client, collection_id.clone()).await {
    ...
    async fn insert_points(
        ...
        for point in points {
        if let Some(qdrant_point_struct) =  construct_point_struct(&point.vector, point.payload, "")
            .await{
            ...
            loop {
                match client
                    .upsert_points_blocking(
                        collection_id.clone(),
                        None,
                        point.clone(), // Ensure point is Clone for retries
                        None,
                    )
                    .await
                {
    ...
    async fn update_points(&self, client: &Self, search_requests: SearchRequest, point: Vec<Point>) -> Result<VectorDatabaseStatus, VectorDatabaseError> {
        todo!()
    }

    async fn delete_points(&self, client: &Self, search_request: SearchRequest) -> Result<CollectionsResult, VectorDatabaseError> {
        todo!()
    }

    async fn get_points(&self, client: &Self, search_request: SearchRequest) -> Result<Vec<Point>, VectorDatabaseError> {
        todo!()
    }

    async fn get_storage_size(&self, client: &Self, search_request: SearchRequest) -> Result<StorageSize, VectorDatabaseError> {
        todo!()
    }
