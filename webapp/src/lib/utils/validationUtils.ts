...
function validateField(object: any, fieldName: string, validations: ValidationUtilOptions, fieldDescriptions, disallowDotSplit?: boolean) {
	...
	if (object) {
		...
		for (let item of values) {
			...
			if (validations.inSet && validations.inSet.size > 0 && (item == null || (Array.isArray(item) ? item.some(x => !validations.inSet.has(x)) : !validations.inSet.has(item)))) {
				error = `${fieldDescription} is an invalid value`;
			}
			if (validations.wholeNumber && (item == null || !isNumber(item) || item % 1 !== 0)) {
				error = `${fieldDescription} is not a valid whole number`;
			}
			if (validations.numberToInclusive && item != null && (!isNumber(item) || item > validations.numberToInclusive)) {
				error = `${fieldDescription} is not a valid number greater than or equal to ${validations.numberToInclusive}`;
			}
			if (validations.numberFromInclusive && item != null && (!isNumber(item) || item < validations.numberFromInclusive)) {
				error = `${fieldDescription} is not a valid number less than or equal to ${validations.numberFromInclusive}`;
			}
			if (validations.regexMatch && item != null && validations.regexMatch instanceof RegExp && !validations.regexMatch.test(item)) {
				error = `${fieldDescription} does not match regular expression ${validations.regexMatch.toString()}`;
			}
			if (validations.regexMatchAll && item != null && validations.regexMatchAll instanceof RegExp && (!Array.isArray(item) || item.length <= 0 || !item.every(x => validations.regexMatchAll.test(x)))) {
				error = `${fieldDescription} does not match regular expression ${validations.regexMatchAll.toString()}`;
			}
			if (validations.startsWith && validations.startsWith.length > 0 && (item == null || !item.startsWith(validations.startsWith))) {
				error = `${fieldDescription} does not start with ${validations.startsWith}`;
			}
			if (validations.endsWith && validations.endsWith.length > 0 && (item == null || !item.endsWith(validations.endsWith))) {
				error = `${fieldDescription} does not end with ${validations.endsWith}`;
			}
			if (validations.contains && validations.contains.length > 0 && (item == null || !item.includes(validations.contains))) {
