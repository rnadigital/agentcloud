...
export async function initRabbit() {
	console.log('rabbitmq_host: %s', process.env.RABBITMQ_HOST);
	console.log('rabbitmq_port: %d', process.env.RABBITMQ_PORT);
	connection = await connect(`amqp://${process.env.RABBITMQ_USERNAME}:${process.env.RABBITMQ_PASSWORD}@${process.env.RABBITMQ_HOST}:${process.env.RABBITMQ_PORT}`);
