...
export const AgentCreatedDisplay = React.memo(({
	...
	const { datasources, tools } = useMemo(() => ({
		datasources: selectedAgentTools?.filter(tool => 
			tool.type === ToolType.RAG_TOOL && selectedAgent?.toolIds?.includes(tool._id)
		) || [],
		tools: selectedAgentTools?.filter(tool => 
			tool.type !== ToolType.RAG_TOOL && selectedAgent?.toolIds?.includes(tool._id)
		) || []
	...
	const agentChanged = JSON.stringify(prevProps.selectedAgent) !== JSON.stringify(nextProps.selectedAgent);
	const toolsChanged = JSON.stringify(prevProps.selectedAgentTools) !== JSON.stringify(nextProps.selectedAgentTools);
