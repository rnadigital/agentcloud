...
const DataSourceGrid = () => {
		...
		return (
			<div className='flex flex-wrap'>
				{Array.from({ length: 6 }).map((_, index) => (
					<div
						key={index}
						className='border-border border grid place-items-center h-44 w-1/3 p-4 bg-muted animate-pulse'>
						<div className='w-full h-full bg-background border border-dashed border-border rounded-md flex justify-center items-center text-muted-foreground flex-col text-sm'>
	...
	return (
		<div className='flex flex-wrap relative'>
			<div className='border-border border grid place-items-center h-44 w-full md:w-1/2 lg:w-1/3 p-4 bg-muted'>
				<div className='w-full h-full bg-background border border-dashed border-border rounded-md flex justify-center items-center text-muted-foreground flex-col text-sm'>
					<ArrowUpTrayIcon className='h-5 w-5' />
					<span>Drag files to upload</span>
					<div className='underline text-primary underline-offset-4'> or browser for files</div>
				</div>
			</div>
			...
			{filteredConnectors.map((connector, index) => (
				<React.Fragment key={connector.name}>
					<button
						className={cn(
							'flex flex-col justify-center items-center border-border border h-44 w-full md:w-1/2 lg:w-1/3 relative',
							{ 'bg-primary/10 border-0': selectedConnectorIndex === index }
						)}
						onClick={() => setSelectedConnector(connector)}>
						<img src={connector.icon} className='h-6 w-6 mb-2' />
						<span className='text-foreground'>{connector.name}</span>
					</button>
					{selectedConnectorIndex === index && selectedConnector && (
