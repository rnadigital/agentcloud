...
function DatasourceStatusIndicator({ datasource }) {
	...
	const finished = (((((success||0)+(failure||0)) >= total) || lastUpdatedAgo > 30000) && total > 0);
	return <span className='tooltip z-100'>
		{total > 0 && <span className='tooltiptext text-sm capitalize !w-[150px] !-ml-[75px] whitespace-pre'>
		...
		{(DatasourceStatus.EMBEDDING === datasource.status && !finished)
			...
			: <div className={`max-w-[300px] px-3 py-[2px] text-sm text-white text-center rounded-full capitalize ${datasource?.status === DatasourceStatus.READY ? datasourceStatusColors[datasource.status] : 'barberpole'}`}>
