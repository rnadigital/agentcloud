...
export default function FunctionRevisionForm({
	revisions,
	tool,
	fetchFormData,
}) {
		...
		{revisions.map((revision, index) => {
			...
			return (<li key={index} className='flex justify-between items-center bg-white shadow rounded p-2'>
				<div className='flex-1 truncate'>
					<p className='text-sm text-gray-800 truncate'>Version {revision._id.slice(-6)} - {new Date(revision.date).toLocaleString()}</p>
				</div>
				{currentRevision
