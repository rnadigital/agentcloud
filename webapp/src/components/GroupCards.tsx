			...
			{groups.map((group) => (
				<li key={group._id} className='rounded-xl border border-gray-200'>
					<div className='flex items-center gap-x-4 border-b border-gray-900/5 bg-gray-50 p-6'>
						...
						<span className='text-ellipsis overflow-hidden whitespace-nowrap'>{group.prompt}</span>
						{/*<img
							src={'/images/favicon.ico'}
							alt={group.name.charAt(0).toUpperCase()}
							className='h-12 w-12 flex-none rounded-lg bg-white object-cover ring-1 ring-gray-900/10 text-center font-bold'
						/>*/}
						<div className='text-sm font-medium leading-6 text-gray-900'>{group.name}</div>
