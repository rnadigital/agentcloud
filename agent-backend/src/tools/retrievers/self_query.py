...
class SelfQueryRetriever(BaseToolRetriever):
    ...
    def perform_query(self, query):
        print(f"metadata_field_info: {self.metadata_field_info}")
        query_result = self.retriever.invoke(query)
        print(f"query result: {query_result}")
        return query_result

    def format_results(self, results):
        return "\n".join(map(lambda x: x if type(x) is str else str({ 'data': x.page_content, 'metadata': x.metadata }), results))
